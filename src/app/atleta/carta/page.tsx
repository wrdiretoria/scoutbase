...
export default function CartaPage() {
  ...
  const checkDesbloqueio = useCallback(async () => {
  ...
  useEffect(() => {
    async function load() {
      ...
      setDesbloqueada(!!(user.user_metadata.carta_desbloqueada))
      ...
      // OVR e atributos — direto da tabela avaliacoes
      try {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const desbloqueou = await checkDesbloqueio()
      if (desbloqueou) clearInterval(interval)
    }, 4000)
    return () => clearInterval(interval)
  }, [polling, checkDesbloqueio])
            ...
            <div style={{ position:'relative', height:280, overflow:'hidden', zIndex:1 }}>
              ...
              {categoria && (
                <div style={{
                  ...
                  padding:'5px 12px',
                  fontSize:10, fontWeight:800, color:'#00FF88', letterSpacing:'0.1em',
                }}>
                  {categoria}
                </div>
              )}
  ...
  return (
    <main style={{
      ...
      {/* Nav */}
      <div style={{ width:'100%', maxWidth:'380px', display:'flex', justifyContent:'space-between', marginBottom:'32px' }}>
        <button onClick={() => router.push('/atleta/perfil')} style={{ background:'none', border:'none', color:'rgba(255,255,255,0.35)', fontSize:'13px', cursor:'pointer', fontWeight:600 }}>
          ← Voltar
        </button>
        <span style={{ fontSize:'13px', fontWeight:800, color:'white' }}>
          ⚽ MEU <span style={{ color:'#00FF88' }}>CRAQUE</span>
      ...
      <div style={{ width:'100%', maxWidth:'380px', display:'flex', flexDirection:'column', gap:'20px' }}>
        ...
        <div className="fu" style={{
          ...
          <div style={{ position:'relative', zIndex:3, textAlign:'center', padding:'32px 20px' }}>
            ...
            <p style={{ margin:'0 0 6px', fontSize:18, fontWeight:900, color:'white' }}>Carta de Avaliação</p>
        ...
        <div className="fu2" style={{
          ...
          {[
            ...
            { icon:'🔒', text:'ID exclusivo verificado MC-XXXXX' },
          ].map(item => (
        ...
        {!pixCode ? (
          <div className="fu3" style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            >
              {gerando ? 'Gerando PIX…' : '⚡ Desbloquear por R$ 5,00'}
            </button>
            {erroPix && (
            ...
            <p style={{ margin:0, fontSize:11, color:'rgba(255,255,255,0.2)', textAlign:'center' }}>
              Pagamento único · PIX instantâneo · Acesso permanente
            </p>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px', borderRadius:12, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)' }}>
              ...
              <p style={{ margin:0, fontSize:13, color:'rgba(255,255,255,0.5)', fontWeight:600 }}>
                Aguardando pagamento… a carta abre automaticamente
              </p>
            </div>
          </div>
        )}
